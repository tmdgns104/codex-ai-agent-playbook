...
- Compiler/source processing의 actual Codex backend, runtime transport, LLM, Ollama, benchmark, external network/API/credential, hardware, cloud write, dependency install, and source script execution count: zero. 완료 절차로 사용자가 명시적으로 요청한 Git push는 별도 control-plane action이다.
