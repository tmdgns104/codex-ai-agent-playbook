...
| 문서 | 내용 |
|---|---|
| [LATEST_STATUS.md](LATEST_STATUS.md) | 현재 V8.3 체크포인트와 다음 재개 지점 |
...
K-Dense 추가 8개 후보는 pinned upstream 정적검사를 완료했고 8/8 `BENCHMARK_READY` 판정이 가능한 상태지만 아직 `inspections.json`에 반영하지 않았습니다.
...
GitHub 첫 화면에서 현재 상태를 바로 확인할 수 있도록 다음 두 README를 2026-08-24 기준으로 갱신했습니다.
...
설치와 일반 사용 명령은 검증된 `main` 기준으로 유지하고, V8.3 Track A/Track B 개발현황과 BENCH-003A 수치는 개발 상태로 별도 표시합니다.
